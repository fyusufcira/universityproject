...
//took username values from tables to check if username exists
    $sqlusername="SELECT user_name from students where user_name='$userName'";
    $resultusername=mysqli_query($conn,$sqlusername);

    $sqlusername="SELECT user_name from admins where user_name='$userName'";
    $resultusernameadmin=mysqli_query($conn,$sqlusername);

    $sqlusername="SELECT user_name from professors where user_name='$userName'";
    $resultusernameprof=mysqli_query($conn,$sqlusername);
    ...
    && mysqli_num_rows($resultusername)==0 && mysqli_num_rows($resultusernameadmin)==0 && mysqli_num_rows($resultusernameprof)==0
        ...
        $result=$db->query("INSERT INTO students (user_name,first_name,last_name,age,email,password,tel_number,is_active)
 VALUES ('$userName','$firstName','$lastName',$age,'$email','$password',$telNumber,1)");

            echo "<script type='text/javascript'>window.alert('Student Succesfully created')</script>";
    }
    //if username exists
...
<div class="wrapper" style="width: 500px; margin-top:20px;!important;">
    ...
    <form action="actionAdminDefineStudent.php" method="post">
        <div class="field">
